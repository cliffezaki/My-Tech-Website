...
export default async function HowStuffWorksArticlePage({ params }: PageProps) {
    ...
    if (!article) {
        const mockExplainer = MOCK_DATA.howStuffWorks.find((item: any) => item.slug === slug || (item.title && item.title.toLowerCase().replace(/ /g, '-') === slug));

        if (mockExplainer) {
            article = {
                title: mockExplainer.title,
                category: mockExplainer.category || "Explainer",
                author: mockExplainer.author || "TechAI Staff",
                publishedAt: new Date().toISOString(),
                readTime: mockExplainer.readTime || "12 min read",
                mainImage: null,
                body: [
                    {
                        _type: 'block',
                        children: [{ _type: 'span', text: mockExplainer.excerpt || "Content coming soon." }]
                    },
                    {
                        _type: 'block',
            ...
            <article className="space-y-8">
                <header className="space-y-4 border-b pb-8">
                    <div className="flex items-center gap-3 text-xs font-semibold uppercase tracking-widest">
                        <span className="text-brand-red">{article.category || "Explainer"}</span>
                        <span className="text-gray-400">·</span>
                        <span className="text-gray-500">{article.readTime || "12 min read"}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-sm text-muted-foreground">
                        <span>By {article.author || "TechAI Staff"}</span>
                        <span>·</span>
